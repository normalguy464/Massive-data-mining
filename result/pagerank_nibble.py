...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Run PageRank Nibble / Approximate Personalized PageRank on the subreddit graph.",
    )
    ...
    parser.add_argument("--min-similarity", type=float, default=0.7361)
...
def load_weighted_graph(csv_path: Path, min_weight: float) -> tuple[dict[str, dict[str, float]], dict[str, float], int]:
    adjacency: dict[str, dict[str, float]] = defaultdict(dict)
    edge_count = 0

    with csv_path.open(newline="", encoding="utf-8-sig") as handle:
        reader = csv.DictReader(handle)
        source_col, target_col, weight_col = detect_edge_columns(reader.fieldnames)
        for row in reader:
            source = str(row.get(source_col, "")).strip()
            target = str(row.get(target_col, "")).strip()
            weight = safe_float(row.get(weight_col), float("nan"))
...
def read_community_info(community_csv: Path, cluster_names_csv: Path) -> dict[str, dict[str, object]]:
...
def read_sorted_subreddits(csv_path: Path, score_column: str) -> list[str]:
    if not csv_path.exists():
        return []
    ...
    with csv_path.open(newline="", encoding="utf-8-sig") as handle:
        for row in csv.DictReader(handle):
            subreddit = str(row.get("subreddit", "")).strip()
            if subreddit:
                rows.append((safe_float(row.get(score_column)), subreddit))
    rows.sort(reverse=True)
    return [subreddit for _, subreddit in rows]


def parse_highway_nodes(value: str) -> list[str]:
    return [item.strip() for item in re.split(r"\s*(?:->|→|â†’)\s*", value or "") if item.strip()]


def read_highway_subreddits(csv_path: Path) -> list[str]:
    if not csv_path.exists():
        return []
    ...
    rows.sort(key=lambda row: safe_int(row.get("rank"), 10**9))
    for row in rows:
        subreddits.extend(parse_highway_nodes(row.get("highway_nodes", "")))
    return subreddits
...
def select_seeds(args: argparse.Namespace, graph_nodes: set[str], degree: dict[str, float]) -> list[str]:
    ...
    bridge = read_sorted_subreddits(args.bridge_csv, "bridge_score")
    gateway = read_sorted_subreddits(args.gateway_csv, "gateway_score_normalized")
    highway = read_highway_subreddits(args.highway_csv)
    high_degree = [node for node, _ in sorted(degree.items(), key=lambda item: item[1], reverse=True)]

    selected: list[str] = []
    seen: set[str] = set()
    for candidate in round_robin([bridge, gateway, highway, high_degree]):
        if candidate in graph_nodes and candidate not in seen:
...
def sweep_best_cluster(
    ...
    order = [
        ...
        for node, score in sorted(
            ppr.items(),
            key=lambda item: item[1] / max(degree.get(item[0], 0.0), 1e-300),
            reverse=True,
        )
    ...
    for index, node in enumerate(order, start=1):
        ...
        if min_cluster_size <= index <= max_cluster_size:
    ...
    if best is None:
        size = min(max(len(order), 1), max_cluster_size)
        cluster = order[:size]
        return cluster, 1.0, 0.0, sum(degree.get(node, 0.0) for node in cluster)

    size, conductance, cut, volume = best
    return order[:size], conductance, cut, volume


def summarize_communities(nodes: list[str], community_info: dict[str, dict[str, object]]) -> str:
    names = []
    seen = set()
    for node in nodes:
        name = str(community_info.get(node, {}).get("community_name") or "")
...
def write_result_csv(
    ...
    community_info: dict[str, dict[str, object]],
    degree: dict[str, float],
    ...
    with args.output_csv.open("w", newline="", encoding="utf-8") as handle:
        ...
        for run in runs:
            ppr = run["ppr"]
            for rank, subreddit in enumerate(run["cluster"], start=1):
                info = community_info.get(subreddit, {})
                weighted_degree = degree.get(subreddit, 0.0)
                ppr_score = ppr.get(subreddit, 0.0)
                writer.writerow(
                    {
                        "seed_subreddit": run["seed"],
                        "rank": rank,
                        "subreddit": subreddit,
                        "community_id": "" if info.get("community_id") is None else info.get("community_id"),
                        "community_name": info.get("community_name", ""),
                        "ppr_score": f"{ppr_score:.12g}",
                        "score_per_degree": f"{(ppr_score / weighted_degree if weighted_degree else 0):.12g}",
                        "weighted_degree": f"{weighted_degree:.12g}",
                        "cluster_size": len(run["cluster"]),
                        "cluster_conductance": f"{run['conductance']:.12g}",
                        "cluster_cut": f"{run['cut']:.12g}",
                        "cluster_volume": f"{run['volume']:.12g}",
                        "alpha": args.alpha,
                        "epsilon": args.epsilon,
                        "min_edge_weight": args.min_similarity,
                    },
                )
...
def write_summary_csv(
    ...
    with args.summary_csv.open("w", newline="", encoding="utf-8") as handle:
        ...
        writer.writeheader()
        for run in runs:
            seed_info = community_info.get(run["seed"], {})
            writer.writerow(
                {
                    "seed_subreddit": run["seed"],
                    "seed_community_id": "" if seed_info.get("community_id") is None else seed_info.get("community_id"),
                    "seed_community_name": seed_info.get("community_name", ""),
                    "cluster_size": len(run["cluster"]),
                    "cluster_conductance": f"{run['conductance']:.12g}",
                    "cluster_cut": f"{run['cut']:.12g}",
                    "cluster_volume": f"{run['volume']:.12g}",
                    ...
                    "top_subreddits": " | ".join(run["cluster"][:12]),
                    "top_communities": summarize_communities(run["cluster"], community_info),
                    "alpha": args.alpha,
                    "epsilon": args.epsilon,
                    "min_edge_weight": args.min_similarity,
                },
...
def write_graph_json(
    ...
    selected: list[str] = []
    seed_by_node: dict[str, list[str]] = defaultdict(list)
    seed_set = {str(run["seed"]) for run in runs}

    for run in runs:
        for node in run["cluster"]:
            if node not in selected and len(selected) < args.graph_node_limit:
                selected.append(node)
            seed_by_node[node].append(run["seed"])
    ...
    for node in selected:
        info = community_info.get(node, {})
        score = max_ppr_by_node[node]
        nodes.append(
            {
                "id": node,
                "label": node,
                "group": "seed" if node in seed_set else f"cluster:{seed_by_node[node][0]}",
                ...
                "title": (
                    ...
                    f"Seed(s): {', '.join(seed_by_node[node][:4])}<br>"
                    f"Community: {info.get('community_name', '')}<br>"
    ...
    edge_payload = [
        {
            ...
            "from": source,
            "to": target,
            "width": round(0.7 + 4.3 * ((weight - args.min_similarity) / max(1.0 - args.min_similarity, 1e-9)), 3),
            "Similarity_Score": round(weight, 6),
    ...
    payload = {
        ...
        "sourceFile": "result/pagerank_nibble_result.csv",
        "extractedFromHtml": False,
        "parameters": {
            "alpha": args.alpha,
            "epsilon": args.epsilon,
            "minEdgeWeight": args.min_similarity,
            "seedCount": len(runs),
        },
        "nodes": nodes,
        "edges": edge_payload,
    ...
    args.graph_json.write_text(json.dumps(payload, ensure_ascii=False, indent=2) + "\n", encoding="utf-8")


def main() -> None:
    args = parse_args()
    adjacency, degree, edge_count = load_weighted_graph(args.graph_csv, args.min_similarity)
    if not adjacency:
    ...
    for seed in seeds:
        ...
        cluster, conductance, cut, volume = sweep_best_cluster(
            adjacency,
            degree,
            ppr,
    ...
    runs.sort(key=lambda run: run["conductance"])
    write_result_csv(runs, community_info, degree, args)
    write_summary_csv(runs, community_info, args)
    write_graph_json(runs, adjacency, degree, community_info, args)
    ...
    print(
        "Best local cluster: "
        f"seed={best['seed']}, size={len(best['cluster'])}, conductance={best['conductance']:.4f}"
    )
